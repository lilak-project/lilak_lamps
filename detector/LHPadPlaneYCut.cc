...
ClassImp(LHPadPlaneYCut)

LHPadPlaneYCut::LHPadPlaneYCut()
:LKPadPlane("LHPadPlaneYCut", "pad plane with rectangular pads following the circle line for LAMPS-TPC")
{
}

bool LHPadPlaneYCut::Init()
{
  if (fPar -> CheckPar("LHPadPlaneYCut/rMinTPC"))      fRMin         = fPar -> GetParDouble("LHPadPlaneYCut/rMinTPC");
  if (fPar -> CheckPar("LHPadPlaneYCut/rMaxTPC"))      fRMax         = fPar -> GetParDouble("LHPadPlaneYCut/rMaxTPC");
  if (fPar -> CheckPar("LHPadPlaneYCut/padGap"))       fPadGap       = fPar -> GetParDouble("LHPadPlaneYCut/padGap");
  if (fPar -> CheckPar("LHPadPlaneYCut/rTopCut"))      fRTopCut      = fPar -> GetParDouble("LHPadPlaneYCut/rTopCut");
  if (fPar -> CheckPar("LHPadPlaneYCut/padWidth"))     fPadWidth     = fPar -> GetParDouble("LHPadPlaneYCut/padWidth");
  if (fPar -> CheckPar("LHPadPlaneYCut/padHeight"))    fPadHeight    = fPar -> GetParDouble("LHPadPlaneYCut/padHeight");
  if (fPar -> CheckPar("LHPadPlaneYCut/radiusLayer0")) fRadiusLayer0 = fPar -> GetParDouble("LHPadPlaneYCut/radiusLayer0");
  if (fPar -> CheckPar("LHPadPlaneYCut/numLayers"))    fNumLayers    = fPar -> GetParInt   ("LHPadPlaneYCut/numLayers");
...
Int_t LHPadPlaneYCut::FindPadID(Int_t section, Int_t row, Int_t layer)
{
...
Int_t LHPadPlaneYCut::FindPadID(Double_t i, Double_t j)
{
...
Double_t LHPadPlaneYCut::PadDisplacement() const
{
  return sqrt(fXSpacing*fXSpacing + fRSpacing*fRSpacing);
}

bool LHPadPlaneYCut::IsInBoundary(Double_t i, Double_t j)
{
...
TH2* LHPadPlaneYCut::GetHist(Option_t *option)
{
  if (fH2Plane != nullptr)
    return fH2Plane; 
...
void LHPadPlaneYCut::DrawFrame(Option_t *)
{
}

TCanvas *LHPadPlaneYCut::GetCanvas(Option_t *)
{
  if (fCanvas == nullptr)
...
LKPad *LHPadPlaneYCut::NewPad(Int_t s, Int_t r, Int_t l)
{
  auto pad = new LKPad();
...
void LHPadPlaneYCut::SetNeighborPads(LKPad *pad0, LKPad *pad1)
{
  pad0 -> AddNeighborPad(pad1);
  pad1 -> AddNeighborPad(pad0);
}

Int_t LHPadPlaneYCut::FindSection(Double_t i, Double_t j)
{
  if (j > fTanPi3o8*i) {
    if (j > fTanPi1o8*i) {
      if (j > fTanPi7o8*i) {
