...
ClassImp(LHPadPlaneFan)

LHPadPlaneFan::LHPadPlaneFan()
:LKPadPlane("PadPlaneF", "pad plane with fan shape pads for LAMPS-TPC")
{
}

bool LHPadPlaneFan::Init()
{
  fInPadArray = new TObjArray();

  fFuncXRightBound = new TF1("RightBound","1/(TMath::Tan(TMath::Pi()*3/8))*x",10,55);

  fRMin = fPar -> GetParDouble("LHPadPlaneFan/rMinTPC");
  fRMax = fPar -> GetParDouble("LHPadPlaneFan/rMaxTPC");

  fNLayerDivision = fPar -> GetParInt("LHPadPlaneFan/nLayerDiv");
  ...
  for (Int_t iDiv = 0; iDiv < fNLayerDivision; iDiv++) {
    fArcLength[iDiv] = fPar -> GetParDouble(Form("LHPadPlaneFan/padInfo/%d",iDiv),0);
    fRadius[iDiv]    = fPar -> GetParDouble(Form("LHPadPlaneFan/padInfo/%d",iDiv),1);
    fNLayers[iDiv]   = fPar -> GetParInt   (Form("LHPadPlaneFan/padInfo/%d",iDiv),2);
  }

  fInDivisionIndex = fPar -> GetParInt("LHPadPlaneFan/innerTrackerDivisionIndex");
  ...
  for (Int_t iSection = 0; iSection < 8; iSection++) {
    for (Int_t iLayer = 0; iLayer < fNLayersTotal; iLayer++) {
      ...
      for (Int_t iRow = -nHalfRows; iRow < nHalfRows; iRow++) {
        ...
        LKPad *pad0 = (LKPad *) fChannelArray -> At(fPadMap[key0]);
        ...
        LKPad *pad1 = (LKPad *) fChannelArray -> At(fPadMap[key1]);
  ...
  for (Int_t iSection = 0; iSection < 8; iSection++) {
    for (Int_t iLayer = 0; iLayer < fNLayersTotal-1; iLayer++) {
      ...
      for (Int_t iRow = -nHalfRowsCurrent; iRow <= nHalfRowsCurrent; iRow++) 
      {
        ...
        LKPad *padCurrent = (LKPad *) fChannelArray -> At(fPadMap[keyCurrent]);
        ...
        for (Int_t iRowNext = -nHalfRowsNext; iRowNext <= nHalfRowsNext; iRowNext++) 
        {
          ...
          LKPad *padNext = (LKPad *) fChannelArray -> At(fPadMap[keyNext]);
...
Int_t LHPadPlaneFan::FindPadID(Int_t section, Int_t row, Int_t layer)
{
...
Int_t LHPadPlaneFan::FindPadID(Double_t i, Double_t j)
{
...
Double_t LHPadPlaneFan::PadDisplacement() const
{
  Int_t max = 0;
...
bool LHPadPlaneFan::IsInBoundary(Double_t i, Double_t j)
{
...
TH2* LHPadPlaneFan::GetHist(Option_t *option)
{
  if (fH2Plane != nullptr)
    return fH2Plane; 
...
void LHPadPlaneFan::DrawFrame(Option_t *)
{
...
TCanvas *LHPadPlaneFan::GetCanvas(Option_t *)
{
  if (fCanvas == nullptr)
...
void LHPadPlaneFan::AddPad(Int_t section, Int_t row, Int_t layer, Double_t i, Double_t j, bool innerTrackerPad = false)
{
  LKPad *pad = new LKPad();
  pad -> SetPosition(i, j);
...
void LHPadPlaneFan::MapPad(LKPad *pad)
{
  std::vector<Int_t> key;
...
Int_t LHPadPlaneFan::FindSection(Double_t i, Double_t j)
{
  if (j > fTanPi3o8*i) {
    if (j > fTanPi1o8*i) {
      if (j > fTanPi7o8*i) {
...
Int_t LHPadPlaneFan::FindDivisionIndex(Int_t layer)
{
  Int_t idx = -1;
  for (Int_t iDiv = 0; iDiv < fNLayerDivision; iDiv++) {
...
Int_t LHPadPlaneFan::FindDivisionIndex(Double_t r)
{
  Int_t idx = -1;
  for(Int_t iDiv=0; iDiv<fNLayerDivision; iDiv++) {
...
void LHPadPlaneFan::ResetHitMap()
{
  LKPadPlane::ResetHitMap();

  fFreeInPadIdx = 0;
}

TObjArray *LHPadPlaneFan::GetInPadArray() { return fInPadArray; }

LKTpcHit *LHPadPlaneFan::PullOutNextFreeHitIn()
{
  if (fFreeInPadIdx == fInPadArray -> GetEntriesFast() - 1)
    return nullptr;

  LKPad *pad = (LKPad *) fChannelArray -> At(fFreeInPadIdx);
  LKTpcHit *hit = pad -> PullOutNextFreeHit();
  if (hit == nullptr) {
    fFreeInPadIdx++;
...
void LHPadPlaneFan::PullOutNeighborHitsIn(LKTpcHits *hits, LKTpcHits *neighborHits)
{
  for (UInt_t iHit = 0; iHit < hits->size(); ++iHit) {
    LKTpcHit *hit = hits -> at(iHit);
    LKPad *pad = (LKPad *) fChannelArray -> At(hit -> GetPadID());
    vector<LKPad *> *neighbors = pad -> GetNeighborPadArray();
    for (UInt_t iNeighbor = 0; iNeighbor < neighbors->size(); ++iNeighbor) {
      LKPad *neighbor = neighbors -> at(iNeighbor);
...
void LHPadPlaneFan::PullOutNeighborHitsIn(TVector3 p, Int_t range, LKTpcHits *neighborHits)
{
  ...
  LKPad *pad = (LKPad *) fInPadArray -> At(id);
  ...
  while (range >= 0) {
    ...
    for (UInt_t iNeighbor = 0; iNeighbor < neighborsNew.size(); ++iNeighbor) {
      LKPad *neighbor = neighborsNew.at(iNeighbor);
      if (FindDivisionIndex(neighbor->GetLayer()) <= fInDivisionIndex)
        neighbor -> PullOutHits(neighborHits);
...
Int_t LHPadPlaneFan::GetNLayerDivision() { return fNLayerDivision; }
Int_t LHPadPlaneFan::GetLayerDiv(Int_t idx) { return fLayerDivI.at(idx); }
Double_t LHPadPlaneFan::GetRDiv(Int_t idx) { return fRDivI.at(idx); }
